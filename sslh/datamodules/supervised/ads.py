...
class AudiosetSupDataModule(LightningDataModule):
    def __init__(
        self,
        root: str = "data",
        ...
        """
        LightningDataModule of AudioSet (Audioset) for partial supervised trainings.
        ...
        if train_subset not in ("balanced", "unbalanced"):
            raise ValueError(
                f'Train subsets available are {("balanced", "unbalanced")}.'
            )
        ...
        self.train_dataset_raw: SingleAudioset = None  # type: ignore
        ...
        if stage == "fit":
            ...
            if self.ratio >= 1.0:
                ...
                indexes_s, _indexes_u = class_balance_split(
                    self.train_dataset_raw, self.ratio, None, verbose=False
                )
