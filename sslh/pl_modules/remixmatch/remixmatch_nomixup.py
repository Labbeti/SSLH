...
class ReMixMatchNoMixUp(ReMixMatch):
    def __init__(
        ...
        criterion_s: nn.Module = NLLLossVecTargets(),
        criterion_u: nn.Module = NLLLossVecTargets(),
        criterion_u1: nn.Module = NLLLossVecTargets(),
        criterion_r: nn.Module = NLLLossVecTargets(),
    ...
    def training_step(
        ...
        probs_xu_weak_and_strong = self.activation(self.model(xu_lst))
        probs_xu1 = self.activation(self.model(xu1_strong))
        probs_r = self.activation_r(self.model.forward_rot(xu1_strong_rotated))  # type: ignore

        loss_s = self.criterion_s(probs_xs_strong, ys)
        loss_u = self.criterion_u(probs_xu_weak_and_strong, yu_lst)
        loss_u1 = self.criterion_u1(probs_xu1, yu1)
        loss_r = self.criterion_r(probs_r, yu1_r)
