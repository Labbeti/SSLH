...
from sslh.pl_modules.uda import (
    UDA,
    UDAMixUp,
...
@hydra.main(config_path=osp.join("..", "conf"), config_name="uda")
def main_uda(cfg: DictConfig) -> None:
    # Initialisation
    reset_seed(cfg.seed)
    if cfg.verbose <= 0:
    ...
    # Build transforms
    transform_identity = get_transform(data_name, cfg.val_aug, **cfg.data.spectro)
    transform_strong = get_transform(data_name, cfg.strong_aug, **cfg.data.spectro)

    train_transform_s = transform_identity
    train_transform_u = UDAUnlabeledPreProcess(transform_identity, transform_strong)
    val_transform = transform_identity
    ...
    elif pl_name == "uda_mixup":
        pl_module = UDAMixUp(**module_params, alpha=cfg.pl.alpha)

    else:
        raise RuntimeError(
            f'Unknown experiment name "{pl_name}". Must be one of {("UDA", "UDAMixUp")}.'
        )
