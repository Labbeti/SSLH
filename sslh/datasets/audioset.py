...
from mlu.datasets.wrappers import TransformDataset, PredicateSubset, DatasetWrapper
from mlu.nn import UnSqueeze
from mlu.transforms import ToTensor, Identity

from sslh.datasets.base import DatasetBuilder
from sslh.datasets.dataset_sized import DatasetSized
from sslh.datasets.detail.as_ds import (
...
class AudioSetBuilder(DatasetBuilder):
	def __init__(self):
		...
		self._data_shape = (320000, )  # (64, 500)
		self._data_key = "waveform"  # "data"
	...
	def get_loader_train(
		...
		**kwargs,
		...
		audioset = dataset
		if isinstance(audioset, DatasetWrapper):
			audioset = audioset.unwrap(recursive=True)
		if not isinstance(audioset, Audioset):
			raise ValueError(
				f"The dataset must be an AudioSet dataset ('{audioset.__class__.__name__}'). "
				f"It can be a wrapped AudioSet in a DatasetWrapper.")
	...
	def get_loaders_split(
		...
		**kwargs,
		...
		balancer_s = batch_balancer(batch_size=batch_size, pool_size=batch_size * 4)
		balancer_u = None
		balancers = [balancer_s, balancer_u]

		loaders = []
		for dataset, num_workers, batch_sampler, balancer in zip(datasets, num_workers_list, batch_samplers, balancers):
			loader = DataLoader(
				dataset=dataset,
				num_workers=num_workers,
