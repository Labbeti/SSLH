...
def class_balance_split(
    ...
    def fill_subset(remaining_samples, expected):
        ...
        subset_occur = np.zeros(shape=(n_classes,))
        subset_indexes = []
        ...
        return np.array(subset_indexes), remaining_samples
    ...
    total_occur = np.array(all_targets).sum(axis=0)
    ...
    u_subset = [s[1] for s in remaining_sample]

    return s_subset.tolist(), u_subset


class IterationBalancedSampler(Sampler):
    def __init__(
        self,
        dataset: ComParE2021PRS,
        index_list: List[int],
        n_max_samples: int,
        shuffle: bool = True,
    ):
