...
def get_prefix(
	args: Namespace, fold_val: Optional[List[int]], interface: DatasetInterface, start_date: str, model_name: str, run_name: str
) -> str:
	"""
	...
	prefix = "{:s}_{:s}_{:s}_{:s}_{:.2f}%_{:s}_{:s}".format(
		interface.get_dataset_name(), start_date, model_name, run_name, su_ratio, fold_suffix, args.tag)
	return prefix
...
def main_run(create_args_fn: Callable, run_fn: Callable, run_name: str):
	...
	if args.cross_validation:
		cross_validation(run_fn, args, start_date, interface, run_name, device)
	else:
		if interface.has_folds():
			if args.folds_train is None and args.folds_val is not None:
				folds_val = args.folds_val
				folds_train = list(set(interface.get_folds()).difference(folds_val))
			elif args.folds_train is not None and args.folds_val is None:
				folds_train = args.folds_train
				folds_val = list(set(interface.get_folds()).difference(folds_train))
			else:
				folds_train = args.folds_train
				folds_val = args.folds_val
		else:
		...
		run_fn(args, start_date, interface, folds_train, folds_val, device)
