...
class Recorder(RecorderABC):
	...
	def add_point(self, key: str, value: float):
		...
		for continues in self._epoch_continues[key].values():
			continues.add(value)
	...
	def is_key_registered(self, key: str) -> bool:
		return key in self._epoch_continues.keys()
	...
	def get_current_means(self) -> Dict[str, float]:
		current_means = {key: continues["mean"].get_current() for key, continues in self._epoch_continues.items()}
		return current_means

	def _update_best_epoch(self, epoch: int):
		# Store final mean of the epoch
		for key, continues in self._epoch_continues.items():
			epoch_mean = continues["mean"].get_current().item()
			if key not in self._means.keys():
				self._means[key] = {}
			self._means[key][epoch] = epoch_mean

			epoch_std = continues["std"].get_current().item()
	...
	def _update_writer(self, epoch: int):
		if self.writer is not None and self.__writer_to_update:
			for key in self._epoch_continues.keys():
