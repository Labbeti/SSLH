...
def main_mean_teacher(cfg: DictConfig) -> None:
    ...
    # Build transforms
    if pl_name in ("mean_teacher", "mean_teacher_cc_mode"):
        ...
        train_aug_names = (
            train_aug_stu_s_name,
            train_aug_stu_u_name,
            train_aug_tea_s_name,
            train_aug_tea_u_name,
        )
        if not all(name == "ident" for name in train_aug_names):
            raise ValueError(
                f"Invalid argument {pl_name=} with augments {train_aug_names}."
            )
    ...
    student = hydra.utils.instantiate(cfg.model)
    teacher = hydra.utils.instantiate(cfg.model)
    optimizer = hydra.utils.instantiate(cfg.optim, params=student.parameters())
