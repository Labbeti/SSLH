...
class ValidaterStack(ValidaterABC):
	def __init__(
		self,
		model: Module,
		activation: Callable,
		...
		"""
			Main class for running a validation that stores all predictions and labels in tensor and call each metric one time.
	...
	def _val_impl(self, epoch: int):
		...
		all_pred = torch.cat(tuple(all_pred))
		all_y = torch.cat(tuple(all_y))

		# Compute metrics
		for metric_name, metric in self.metrics.items():
			score = metric(all_pred, all_y)
			self.recorder.add_scalar(metric_name, score)

		self.printer.print_current_values(self.recorder.get_current_means(), 0, len(self.loader), epoch, self.name)
