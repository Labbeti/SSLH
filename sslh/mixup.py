...
from sslh.pl_modules.mixup import (
    MixUp,
    MixUpMixLabel,
)
...
@hydra.main(config_path=osp.join("..", "conf"), config_name="mixup")
def main_mixup(cfg: DictConfig) -> None:
    # Initialisation
    reset_seed(cfg.seed)
    if cfg.verbose <= 0:
    ...
    elif pl_name == "mixup_mix_label":
        pl_module = MixUpMixLabel(**module_params)

    else:
        raise RuntimeError(
            f'Unknown experiment name "{pl_name}". Must be one of {("MixUp", "MixUpMixLabel")}.'
        )
