...
class CutMixSpec(Module):
    ...
    def forward(
        self, batch_a: Tensor, batch_b: Tensor, label_a: Tensor, label_b: Tensor
    ) -> (Tensor, Tensor):
        """
    ...
    def gen_rand_bbox(
        self, width: int, height: int, lambda_: Tensor
    ) -> (Tensor, Tensor, Tensor, Tensor):
        cut_ratio = math.sqrt(1.0 - lambda_)

        # rw, rh, rx, ry in paper
        cut_width = torch.scalar_tensor(math.floor(width * cut_ratio)).int()
        cut_height = torch.scalar_tensor(math.floor(height * cut_ratio)).int()
