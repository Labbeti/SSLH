...
class MixUpRoll(Module):
    ...
    def forward(
        ...
        if batch_a.shape != batch_b.shape:
            raise RuntimeError(
                f"Mismatch between batch shapes for MixUpRoll : ({batch_a.shape} != {batch_b.shape})"
            )
        if labels_a.shape != labels_b.shape:
            raise RuntimeError(
                f"Mismatch between label shapes for MixUpRoll : ({labels_a.shape} != {labels_b.shape})"
            )
        ...
        shift_a = (
            torch.floor(batch_a.shape[self.dim_roll] * self._uniform.sample())
            .int()
            .item()
        )
