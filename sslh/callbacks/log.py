...
class LogCallback(Callback, ABC):
    def __init__(self, log_on_epoch: bool = True) -> None:
        super().__init__()
        self.log_on_epoch = log_on_epoch

    def _log_impl(self, pl_module, **kwargs) -> None:
...
class LogTensorMemoryCallback(LogCallback):
    def __init__(self, prefix: str = "train/", log_on_epoch: bool = True) -> None:
        super().__init__(log_on_epoch)
        self.prefix = prefix

    def _log_impl(self, pl_module: LightningModule, **kwargs) -> None:
        allocated = sum(
...
class LogLRCallback(LogCallback):
    ...
    def _log_impl(self, pl_module: LightningModule, **kwargs) -> None:
        ...
        if not isinstance(optimizer, Optimizer):
            raise TypeError(f"Cannot log lr with optimizers type {type(optimizer)=}.")
...
class LogPLAttrCallback(LogCallback):
    def __init__(
        self, attr_name: str, prefix: str = "train/", log_on_epoch: bool = True
    ) -> None:
        super().__init__(log_on_epoch)
