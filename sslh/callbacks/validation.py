...
class ValidationCallback(Callback):
	def __init__(
		...
		**forward_kwargs,
		...
		if isinstance(val_check_interval, int) and val_check_interval >= 0:
			...
			raise RuntimeError('Parameter "val_check_interval" must be a positive integer or float.')
	...
	def on_train_batch_end(
		...
		if self._val_step_interval != 0 and global_step % self._val_step_interval == self._val_step_interval - 1:
	...
	def run(self, trainer: Trainer, pl_module: LightningModule):
		with torch.no_grad():
			...
			if len(dataloaders) == 0:
				...
				raise RuntimeError(f'Unsupported multiple dataloaders for {self.__class__.__name__}.')
	...
	def run_validation(self, dataloader: DataLoader, pl_module: LightningModule):
		...
		for batch in dataloader:
			...
			pred_xs = pl_module(xs, **self.forward_kwargs)
			scores = self.metric_dict_val(pred_xs, ys)
