...
from sslh.pl_modules.deep_co_training import (
    DeepCoTraining,
)
...
@hydra.main(config_path=osp.join("..", "conf"), config_name="deep_co_training")
def main_deep_co_training(cfg: DictConfig) -> None:
    # Initialisation
    reset_seed(cfg.seed)
    if cfg.verbose <= 0:
    ...
    # Build transforms
    train_transform_s = get_transform(data_name, cfg.aug_s, **cfg.data.spectro)
    train_transform_u = get_transform(data_name, cfg.aug_u, **cfg.data.spectro)
    ...
    model_f = hydra.utils.instantiate(cfg.model)
    model_g = hydra.utils.instantiate(cfg.model)
    all_params = itertools.chain(model_f.parameters(), model_g.parameters())
    activation = get_activation_from_name(cfg.pl.activation)
    log_activation = get_activation_from_name(cfg.pl.log_activation)
    optimizer = hydra.utils.instantiate(cfg.optim, params=all_params)

    criterion_s = get_criterion_from_name(
        cfg.pl.criterion_s, cfg.pl.reduction, log_input=True
    )
    ...
    module_params: dict[str, Any] = dict(
        ...
        epsilon=cfg.pl.epsilon,
        lambda_cot=cfg.pl.lambda_cot,
        lambda_diff=cfg.pl.lambda_diff,
        train_metrics=train_metrics,
        val_metrics=val_metrics,
        log_on_epoch=cfg.data.log_on_epoch,
    )

    if pl_name == "deep_co_training":
