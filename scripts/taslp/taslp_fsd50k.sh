...
dataset="fsd50k"
val_check_interval=1.0 # 1000
n_train_steps=null # 25000
sampler_s_balanced=false
log_on_epoch=true # false
...
rd="${RANDOM}"
tag_prefix="_taslp_${dataset}_${model_acro}_${rd}"
dataset_params="dataset=${dataset} dataset.val_check_interval=${val_check_interval} dataset.n_train_steps=${n_train_steps} dataset.sampler_s_balanced=${sampler_s_balanced} dataset.log_on_epoch=${log_on_epoch}"
common_params="${run} ${dataset_params} epochs=${epochs} expt.activation=${activation} gpus=${gpus} cpus=${cpus} trainer.accelerator=${accelerator} verbose=${verbose} debug=${debug} optim.lr=${lr} sched=${sched}"
