...
from typing import Any, Callable, Generic, List, Optional, TypeVar, Tuple, Union
...
class RandomApplyWrap(Transform, Generic[T, U]):
    def __init__(self, callable_: Callable[[T], U], p: float = 1.0) -> None:
        super().__init__(p=p)
        self.callable = callable_

    def process(self, x: T) -> U:
...
class TensorInternalWrap(ProcessWrap):
    def __init__(self, pil_transform: ImageTransform, p: float = 1.0) -> None:
        """
    ...
    def process(self, x: Image.Image) -> Image.Image:
        self.post_convert.mode = x.mode  # type: ignore
        return self.post_convert(self.transform(self.pre_convert(x)))  # type: ignore
...
class Duplicate(Transform):
    ...
    def process(self, x: Any) -> Any:
        if isinstance(self.n, tuple):
            n = int(torch.randint(self.n[0], self.n[1], ()).item())
        else:
            n = self.n
