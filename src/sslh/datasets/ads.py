...
"""
AudioSet (Audioset) core classes and functions.
...
class Audioset(Dataset):
    ...
    def __init__(
        ...
        self.hdf_chunk_size: Optional[int] = None
    ...
    def _errors(self):
        ...
        if self.hdf_chunk_size is None or not is_power2(self.hdf_chunk_size):
            raise RuntimeError(
                "HDF file chunk first dimension must be a power of 2. it is %s"
                % self.hdf_chunk_size
            )
    ...
    def _check_hdf(self):
        ...
        if not os.path.isfile(audioname_path):
            raise RuntimeError(
                f"Audioname standalone file doesn't exist. Please create it. (audioname_path={audioname_path})"
            )

        if not os.path.isfile(targets_path):
            raise RuntimeError(
                f"Targets standalone file doesn't exist. Please create it. (targets_path={targets_path})"
            )
    ...
    def _prepare_hdfs(self):
        ...
        def get_chunk_valid_stat(hdf_file_: h5py.File):
            n_row_ = len(hdf_file_["audio_name"])  # type: ignore
            chunk_size = hdf_file_["audio_name"].chunks[0]  # type: ignore
    ...
    def __getitem__(self, sample_idx: int) -> Tuple[Tensor, Tensor]:
        ...
        """
        if self.hdf_chunk_size is None:
            raise RuntimeError(
                f"Dataset is not correctly initialized. (found {self.hdf_chunk_size=} but expected an integer)"
            )
    ...
    def _read_chunk(
        self, hdf_file: h5py.File, chunk_idx: int
        ...
        hdf_file["target"].read_direct(targets, slice(start, end), None)  # type: ignore
        hdf_file[self.data_key].read_direct(waveforms, slice(start, end), None)  # type: ignore
    ...
    def get_data(self, sample_idx: int):
        ...
        return hdf_file[self.data_key][hdf_sample_idx]  # type: ignore
    ...
    def get_target(self, sample_idx: int):
        ...
        hdf_sample_idx = sample_idx % self.hdf_n_row[hdf_name]
        return hdf_file["target"][hdf_sample_idx]  # type: ignore
...
class SingleBalancedSampler(Sampler):
    ...
    def _sort_per_class(self):
        ...
        for idx, (sample_idx, target) in enumerate(
            zip(self.index_list, self.all_targets)
        ):
        ...
        empty_classes = [
            cls_idx
            for cls_idx, indexes in enumerate(class_indexes)
            if len(indexes) == 0
        ]
        if len(empty_classes) > 0:
            logging.warning(
                f"Found at least 1 class without any indexes. (classes={str(empty_classes)})"
            )
...
def get_class_statistic(
    dataset: Audioset, batch_indexes: List[int]
