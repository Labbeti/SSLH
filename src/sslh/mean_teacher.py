...
from sslh.pl_modules.mean_teacher.preprocess import MeanTeacherAugPreProcess
...
@hydra.main(config_path=osp.join("..", "conf"), config_name="mean_teacher")
def main_mean_teacher(cfg: DictConfig) -> None:
    # Initialisation
    reset_seed(cfg.seed)
    if cfg.verbose <= 0:
    ...
    # Build transforms
    if pl_name in ("mean_teacher", "mean_teacher_mixup"):
        # MT with augments (slower by default)
        train_transform_stu_s = get_transform(
            data_name, cfg.train_aug_stu_s, **cfg.data.spectro
        )
        train_transform_stu_u = get_transform(
            data_name, cfg.train_aug_stu_u, **cfg.data.spectro
        )
        train_transform_tea_s = get_transform(
            data_name, cfg.train_aug_tea_s, **cfg.data.spectro
        )
        train_transform_tea_u = get_transform(
            data_name, cfg.train_aug_tea_u, **cfg.data.spectro
        )

        train_transform_s = MeanTeacherAugPreProcess(
            train_transform_stu_s, train_transform_tea_s
        )
        train_transform_u = MeanTeacherAugPreProcess(
            train_transform_stu_u, train_transform_tea_u
        )

    elif pl_name in ("mean_teacher_same_aug", "mean_teacher_cc_mode"):
        # Default MT cannot use augments
        train_aug_stu_s_name = hydra_cfg.runtime.choices["aug@train_aug_stu_s"]
        train_aug_stu_u_name = hydra_cfg.runtime.choices["aug@train_aug_stu_u"]
        train_aug_tea_s_name = hydra_cfg.runtime.choices["aug@train_aug_tea_s"]
        train_aug_tea_u_name = hydra_cfg.runtime.choices["aug@train_aug_tea_u"]

        if train_aug_stu_s_name != train_aug_tea_s_name:
            raise ValueError(
                f"Invalid sup augmentations {train_aug_stu_s_name=} != {train_aug_tea_s_name=}."
            )

        if train_aug_stu_u_name != train_aug_tea_u_name:
            raise ValueError(
                f"Invalid sup augmentations {train_aug_stu_u_name=} != {train_aug_tea_u_name=}."
            )

        train_transform_s = get_transform(
            data_name, cfg.train_aug_stu_s, **cfg.data.spectro
        )
        train_transform_u = get_transform(
            data_name, cfg.train_aug_stu_u, **cfg.data.spectro
        )

    else:
        raise ValueError(f"Invalid argument {pl_name=}.")
    ...
    student = instantiate(cfg.model)
    if cfg.tea_has_same_weights:
        teacher = copy.deepcopy(student)
    else:
        teacher = instantiate(cfg.model)
    optimizer = instantiate(cfg.optim, params=student.parameters())
