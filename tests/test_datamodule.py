...
from sslh.transforms.get.get_from_name import get_transform


@hydra.main(config_path=osp.join("..", "config"), config_name="supervised")
def main(cfg: DictConfig) -> None:
    train_transform = get_transform(data_name, "identity", **cfg.data.spectro)
    ...
    for batch in tqdm.tqdm(train_dataloader):
        ...
        assert list(n_zeros_xs.shape) == [
            cfg.bsize
        ], f"{list(n_zeros_xs.shape)} != {[cfg.bsize]}"
        assert list(n_zeros_ys.shape) == [
            cfg.bsize
        ], f"{list(n_zeros_ys.shape)} != {[cfg.bsize]}"
