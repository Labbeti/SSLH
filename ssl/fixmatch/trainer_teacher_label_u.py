...
class FixMatchTrainerTeacherLabelU(FixMatchTrainer):
	def __init__(
		self,
		model: Module,
		activation: Callable,
		...
		self.metrics_u_true = {f"{name}_true": copy.deepcopy(metric) for name, metric in metrics_u.items()}
	...
	def _train_impl(self, epoch: int):
		...
		for i, ((batch_s_augm_weak, labels_s), (batch_u_augm_weak, batch_u_augm_strong, true_labels_u)) in enumerate(self.loader):
			...
			true_labels_u = true_labels_u.to(self.device).float()
			...
			with torch.no_grad():
				...
				for metric_name, metric in self.metrics_u_true.items():
					_mean = metric(pred_u_augm_strong, true_labels_u)
					self.recorder.add_point("train/{:s}".format(metric_name), metric.value.item())
		...
		self.recorder.add_point("train/lambda_u", self.lambda_u)
