...
from ssl.remixmatch.average_pred import AveragePred
from ssl.remixmatch.loss import ReMixMatchLoss
from ssl.remixmatch.module_rot import ModuleRot
...
class ReMixMatchTrainer(MixMatchTrainer):
	...
	def _train_impl(self, epoch: int):
		self.model.train()
		for metrics in (self.metrics_s_mix.values(), self.metrics_u_mix.values(), self.metrics_u1.values(), self.metrics_r.values()):
		...
		for i, ((batch_s_augm_strong, labels_s), (batch_u_augm_weak, batch_u_augm_strong_multiple)) in enumerate(
			...
			with torch.no_grad():
				...
				self.recorder.add_point("train/loss_u1", loss_u1.item())
				self.recorder.add_point("train/loss_r", loss_r.item())
				...
				for metric_name, metric in self.metrics_u1.items():
					_mean = metric(pred_u1, labels_u1)
					self.recorder.add_point("train/{:s}".format(metric_name), metric.value.item())

				for metric_name, metric in self.metrics_r.items():
					_mean = metric(pred_r, labels_r)
					self.recorder.add_point("train/{:s}".format(metric_name), metric.value.item())
